...
int main(int argc, char** argv) {
  if (argc != 6) {
    std::cout << "Usage: " << argv[0]
              << " nsteps dump_freq input.csv particle_type timestep yolo"
              << std::endl;
    std::cout << "\tparticle type can be: planet, ping_pong and you are in the right folder" << std::endl;
  ...
  std::cout << " Instance obtained"<< std::endl;

  SystemEvolution& evol = factory.createSimulation(filename, timestep);

  std::cout << evol.getSystem().getParticle(0).getVelocity()<< std::endl;
